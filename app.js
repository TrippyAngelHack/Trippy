...
//            console.log(timesWent);
            ...
            evaluate(popularSeason, vibe, description, uber, uber, getAge(birthday), drink, likelyBeen, placesToGo);
            
            var postMessage = getFirstMessageFeedback(feedData);
            console.log("Message of Post:")
            console.log(postMessage);
            console.log(getSentiment(postMessage));
        }
    );
}
...
function getFirstMessageFeedback(feedData)
{
    var likelyBeen = [];
    for(var i = 0; i < feedData.length; ++i)
    {
...
/**/
var HPE_KEY = '2a1036c9-d71b-48f1-adbc-3d1d8c531e20';
...
function getURL(words){
 var SENTIMENT_URL = 'https://api.havenondemand.com/1/api/sync/analyzesentiment/v1?text=';
 var url = SENTIMENT_URL + words + '&apikey=' + HPE_KEY;
    return url;
}

function getSentiment (words){
 var sentimentJSON = getJSON(getURL(words));
 var result = sentimentJSON.aggregate.sentiment;
 return result;
}